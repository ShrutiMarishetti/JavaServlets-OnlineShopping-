...
<%-- <%@page import="com.deloitte.util.Util"%> --%>
...
%>    
 --%>    
 <%
List<CategoryEntity> categorySession = (List<CategoryEntity>) session.getAttribute("category");
%>
<%
List<ProductEntity> productSession = (List<ProductEntity>) session.getAttribute("product");
%>
	...
	<body>
		...
		<%
			...
			if(null == catId || "".equals(catId) || catId.trim().equals("All")){
				...
				pl = pd.getAllProducts(catTitle);
			}    */
			
			CategoryDAO cd = new CategoryDAO();
			List<CategoryEntity> cl = cd.getAllCategories();
		%> 
				...
				<div class="col-md-2">
					...
					<div class="list-group">
						...
						<a href="homepage.jsp?category=All" class="list-group-item list-group-item-action  <%=isAllProductsActive%> " aria-current="true">
						  ...
						  <%-- <h6 style="color:blue"> Categories:<%=cl.size()%></h6>  --%>
						
						<%	String categoryActive;
							for(CategoryEntity c: cl){								
								if( !isAllProductsActive.equals("active") && c.getCategoryName()==request.getParameter("catTitle")) {
						...
						%>
								<a href="homepage.jsp?category= <%=c.getCategoryId()%>" class="list-group-item list-group-item-action <<%=categoryActive%>>"> <%=c.getCategoryName()%> </a>
												...
												<div class="card product-card">
													...
													<div class="card-footer text-center">
														...
														<button class="btn btn-outline-success">
															&#8377; <%=p.getProductDiscount()%><br> 
															<span class="text-secondary original-price"><%=p.getProductPrice()%></span>/- <br>
